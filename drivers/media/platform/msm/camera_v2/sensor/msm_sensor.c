	...
	if (s_ctrl->sensordata->vendor_id_info->vendor_id != vendorid) {
		...
		if (have_vcmid == 1) {
			if (s_ctrl->sensordata->vcm_id_info->vcm_id != vcmid) {
				pr_err("%s:match vcmid if failed read vcm id:\
						0x%x expected id 0x%x:\n",
						__func__, vcmid,
						s_ctrl->sensordata->vcm_id_info->vcm_id);
				rc = -1;
				return rc;
			} else {
		...
		if (have_lensid == 1) {
			if (s_ctrl->sensordata->lens_id_info->lens_id != lensid) {
				pr_err("%s:match lensid if failed read lens id:\
						0x%x expected id 0x%x:\n",
						__func__, lensid,
						s_ctrl->sensordata->lens_id_info->lens_id);
				rc = -1;
				return rc;
			} else {
